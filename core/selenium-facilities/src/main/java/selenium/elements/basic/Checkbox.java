...
public class Checkbox extends AbstractElement{
    ...
    public Checkbox ( SearchContext parent, By by, String name ){
    ...
    public Checkbox ( Element parent, By by, String name ){
        super( parent, by, name );
        this.setElementName( STR_ELEMENT_NAME );
    }
    ...
    public ResultList setValue ( String value ) throws TestExecutionRuntimeException{
        ...
        ResultList result = new ResultList( String.format( Checkbox.STR_MSG_SET_CHECKBOX, this._name, value ) );
        try {
            if ( !this.getValue().equals( value ) ){
                result.push( this.click(), true );
            }
            else {
                result.push( new Result( StepExStatus.passed, String.format( Checkbox.STR_MSG_CHECKBOX_ALREADY_SET, this._name, value ) ), true );
            }
        }
        catch ( Exception ex ){
            result.push( ex, true );
